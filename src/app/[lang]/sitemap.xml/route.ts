...
export async function GET(
  ...
  const urls: { loc: string; priority: string; changefreq: string }[] = [
    ...hubs.map(h => ({ loc: `https://www.colormenow.shop/${lang}/${h.slug}`, priority: '0.9', changefreq: 'weekly' })),
    ...themes.map(t => ({ loc: `https://www.colormenow.shop/${lang}/${t.parentHub}/${t.slug}`, priority: '0.8', changefreq: 'weekly' })),
    ...agePages.map(a => ({ loc: `https://www.colormenow.shop/${lang}/${a.parentHub}/${a.parentTheme}/${a.ageGroup}`, priority: '0.7', changefreq: 'weekly' })),
    ...pages.map(p => ({ loc: `https://www.colormenow.shop/${lang}/${p.parentHub}/${p.parentTheme}/${p.ageGroup}/${p.slug}`, priority: '0.6', changefreq: 'monthly' })),
  ];
